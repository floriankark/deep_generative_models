...
class Encoder(nn.Module):
    def __init__(self, channels: list[int], latent_dim: int):
        ...
        self.fc_logvar = nn.Linear(in_features=config["fc_output"], out_features=latent_dim)
...
class Decoder(nn.Module):
    def __init__(self, channels: list[int], latent_dim: int):
        ...
        self.up_conv = nn.ModuleList(
            [nn.ConvTranspose2d(channels[i], channels[i], **config["up_conv"]) for i in range(len(channels) - 1)]
        )
    ...
    def forward(self, x):
        x = self.relu(self.fc(x)).reshape(-1, self.channels[0], config["fc_input"], config["fc_input"])
